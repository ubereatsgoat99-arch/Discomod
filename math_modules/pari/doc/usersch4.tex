...
general presentation of the system, of the \kbd{gp} calculator, and describes
high level PARI routines available through the calculator. The
present manual focuses on the usage of the PARI library, assuming
general familiarity with the contents of these chapters and with ANSI C
programming. In the present chapter, we introduce concepts and describe
useful general purpose functions; the following chapters describes all public
low or high-level functions, underlying or extending the GP functions seen in
...
\item a table of primes, called \tet{pari_PRIMES}, of type \kbd{pari\_prime*}
...
\subsec{Backward compatibility} The PARI function names evolve over time,
...
one (provided the function prototype remained the same).
...
\kbd{GEN}s, and creates the result $x+y$ on the PARI stack. For most
...
Low level kernel functions implement these operators for specialized
arguments and are also available: Level 0 deals with operations at the word
level (\kbd{long}s and \kbd{ulong}s), Level 1 with \typ{INT} and \typ{REAL}
and Level 2 with the rest (modular arithmetic, polynomial arithmetic and
linear algebra). Here are some examples of Level $1$ functions:
...
long integer, {\bf u} for an unsigned C long integer. Chapter 6 gives a
...
There, it is replaced by the simpler rule: the result is a \typ{INT} if
...
\settabs\+ -----------------------------&---------------&------------\cr \+
\tet{DEFAULTPREC}   & 64       & ($\approx$ 19 decimal digits) \cr\+
\tet{MEDDEFAULTPREC}& 128      & ($\approx$ 38 decimal digits) \cr\+
\tet{BIGDEFAULTPREC}& 192      & ($\approx$ 57 decimal digits) \cr
\noindent For instance, suppose you call a transcendental function, such as

\kbd{GEN \key{gexp}(GEN x, long prec)}.

\noindent The last argument \kbd{prec} is an integer multiple of $\tet{BITS_IN_LONG}$,
corresponding to the default floating point precision required, given in bits.
It is \emph{only} used if
\kbd{x} is an exact object, otherwise the relative precision is determined by
the precision of~\kbd{x}. Note that for parity reasons, half the accuracies
...
concrete examples in the next subsections.
...
restored. Think of the \kbd{set\_avma} call as a simple \kbd{avma = av}
restoring the \kbd{avma} value.
...
\noindent\item
If on the contrary, you want to keep the latest computed object $z$ and destroy
everything else since the reference point, use

\bprog
  pari_sp av = avma; /*@Ccom record initial avma */
  ... garbage ...
  oldz = fun();
  newz = gc_upto(av, oldz); /*@Ccom destroy garbage but keep z safe */
@eprog
Of course, this can be written directly as
\bprog
  z = gc_upto(av, fun());
@eprog\noindent This form is actually preferred since it guarantees garbage
collection occurs at the right time, just after \kbd{fun()} returns its
result. Note that the \kbd{gc\_upto} call moves \kbd{oldz} and in fact
\emph{destroys} it. One should never refer to it again, but use the updated
\kbd{newz} instead.

\noindent\item
When an arbitrary \kbd{GEN} must be recovered from the middle of a
computation, even if it was constructed incrementally, then you can use
the most robust (hence slowest) garbage collection function: \tet{gc_GEN}.
\bprog
  pari_sp av = avma; /*@Ccom record initial avma */
  ... garbage ...
  oldz = fun();
  ... garbage ...
  newz = gc_GEN(av, oldz); /*@Ccom destroy garbage but keep z safe */
@eprog\noindent This function even allows $z$ to contain shallow data
(pointers to other parts of the stack), which is forbidden for high-level
PARI functions. Robustness comes at a price but this is at most twice slower
than \kbd{gc\_upto}. Again \kbd{oldz} should not be accessed anymore.

\noindent\item
Finally, when more that one result needs to be saved, you can use the
following idiom:
\bprog
  pari_sp av = avma; /*@Ccom record initial avma */
  ... garbage ...
  ...
  z2 = fun();
  ... garbage ...
  gc_all(av, 2, &z1, &z2); /*@Ccom destroy garbage, keep z1 and z2 safe */
@eprog\noindent As usual, \kbd{gc\_all} changes \kbd{z1} and \kbd{z2}:
the old values should no longer be accessed. This is a variadic function,
which essentially performs as many simultaneous \kbd{gc\_GEN} as needed: if
$n$ objects need to be updated, use
\bprog
  gc_all(av, n, &z1, ..., &zn);
@eprog\noindent

\subsec{Gory technical details}

You may skip this section on first reading. We will now describe
the underlying mechanisms in terms of the lowest level function

\fun{size_t}{gc_stack_update}{pari_sp ltop, pari_sp lbot}. This function
cleans up the stack between \kbd{ltop} and \kbd{lbot}, where
$\kbd{lbot} \leq \kbd{ltop}$, and returns the offset $o = \kbd{ltop} -
\kbd{lbot}$. This means the following.

1) we translate (copy) all data in the interval $[\kbd{avma}, \kbd{lbot}[$
to $[\kbd{avma} + o, \kbd{lbot} + o]$. Note that the right extremity
is now exactly \kbd{ltop}. Graphically
...
2) The function then inspects all the PARI objects between (the original)
\kbd{avma} and \kbd{lbot}, i.e.~the ones that we want to keep and that have
been translated. Every component of such an object which is not a codeword is
a pointer to an address which is either

-- between \kbd{avma} and \kbd{lbot}, in which case we add to it the offset~$o$;

-- larger than or equal to \kbd{ltop}, in which case it does not change, or

-- between \kbd{lbot} and \kbd{ltop} in which case the function
raises an error: ``significant pointers lost in gc''.

3) \key{avma} is replaced by $\key{avma} + o$ as in the diagram above.

4) The function returns the offset $o$.

As stated above, no component of the remaining objects should belong to the
erased segment [\kbd{lbot}, \kbd{ltop}[, and the function checks this.
But beware as well that the addresses of the objects in the translated zone
have now changed, so you must not access any pointer which previously pointed
into the zone below \kbd{ltop}.

\subsec{The basic 3 garbage collecting functions}

Let us recap: although all is implemented in terms of the
\kbd{gc\_stack\_update} low level function, you don't want to use it directly.
You mostly want to use one of the following three functions to collect
garbage (reclaim memory) while keeping some results safe. Many others will be
described in \secref{se:garbage2} for specialized scenarios.

\fun{GEN}{gc_upto}{pari_sp ltop, GEN q}. This applies when \kbd{q} is the last
object on the stack, returned by a high level PARI function. It cleans the
stack between \kbd{ltop} and the object \kbd{q} and returns \kbd{q} updated.
Your own functions or low level PARI functions (such as shallow functions)
described in the next Chapter may not satisfy this: this will be mentioned in
their documentation.

\fun{GEN}{gc_GEN}{pari_sp ltop, GEN q}. Works in all cases. Functionally
equivalent to, but more efficient than
\bprog
  gc_upto(ltop, gcopy(q))
@eprog\noindent In this case, the \kbd{GEN} parameter \kbd{q} need not
satisfy any property before the garbage collection. Of course, this is about
twice slower than \kbd{gc\_upto}, because \kbd{q} has to be copied to a clean
stack zone first. This function is a special case of \kbd{gc\_all} below,
where $n=1$.

\fun{GEN}{gc_all}{pari_sp ltop, int n, ...}.
...
\bprog
  ...
  garbage(); x = anything();
  garbage(); y = anything();
  garbage(); gc_all(ltop, 2, &x, &y);
@eprog\noindent
It cleans up the most recent part of the
stack (between \kbd{ltop} and \kbd{avma}), updating all the \kbd{GEN}s added
to the argument list. This is as robust as \kbd{gc\_GEN}. For convenience,
this function also returns the new value of $x$, the first object recovered.

\subsec{A first detailed example}
...
the stack contains as unnecessary garbage \kbd{x2} and \kbd{y2}.
More precisely it contains (in this order) \kbd{x2}, \kbd{y2}, \kbd{z}.
...
\kbd{cgiv} since \kbd{x2} and \kbd{y2} are not the latest computed objects
and we do not want to give back~\kbd{z}.

Using \kbd{gc\_upto}, we can give back the memory locations
corresponding to \kbd{x2}, \kbd{y2}, and move the object \kbd{z} upwards so
that no space is lost. Specifically:
\bprog
  pari_sp ltop = avma;  /*@Ccom remember the current top of the stack */
  GEN x2 = gsqr(x);
  GEN y2 = gsqr(y);
  GEN z = gc_upto(ltop, gadd(x2, y2));
@eprog\noindent or even
\bprog
  ltop = avma;
  z = gc_upto(ltop, gadd(gsqr(x), gsqr(y)));
@eprog\noindent This works because the output of \kbd{gadd} is the last
object on the stack and everything else that was introduced after recording
the \kbd{ltop} reference point is deleted. Another possibility is to use
\bprog
  ltop = avma;
  z = gc_GEN(ltop, gadd(gsqr(x), gsqr(y)));
@eprog\noindent which would work in all cases, even if \kbd{gadd} were
shallow. Garbage collection would be at most twice slower, and certainly
negligible compared to the squaring times. Make sure you understand exactly
what has happened before you go on!

\subsec{More examples: variations on a theme} it is often necessary to do
several garbage collections during a computation. However, the fewer the
better. The only condition for \kbd{gc\_upto} to work is that the
garbage be connected and we need only recover the last result on the stack.
The computation should be arranged so that there is a minimal number of
connected pieces of garbage.
...
%
\bprog
  x2 = gsqr(x); a = gadd(x2, y);
  y2 = gsqr(y); b = gadd(y2, x);
  z = mkvec2(a, b);  /* shallow: not suitable for gc_upto! */
@eprog\noindent
This leaves a dirty stack containing \kbd{x2}, \kbd{a},
\kbd{y2}, \kbd{b} and \kbd{z}. The garbage here consists of \kbd{x2} and
\kbd{y2}, which are separated by \kbd{a}, which we need to keep. But if we
compute \kbd{y2} \emph{before} \kbd{a} then the garbage becomes connected,
and we get the following program with garbage collecting:
%
\bprog
  ltop = avma; x2 = gsqr(x); y2 = gsqr(y);
  z = cgetg(3, t_VEC);
  gel(z, 1) = gadd(x2,y);
  gel(z, 2) = gadd(y2,x); z = gc_upto(ltop, z);
@eprog\noindent Beware that
\bprog
  ltop = avma; a = gadd(gsqr(x), y); b = gadd(gsqr(y), x);
  z = cgetg(3, t_VEC);
  gel(z, 1) = a;
  gel(z, 2) = b; z = gc_upto(ltop,z); /*@Ccom WRONG */
@eprog\noindent
is a disaster since \kbd{a} and \kbd{b} are created before
\kbd{z}, so the call to \kbd{gc\_upto} overwrites them, leaving
\kbd{gel(z, 1)} and \kbd{gel(z, 2)} pointing at random data!

On the other hand
\bprog
  ltop = avma; z = cgetg(3, t_VEC);
  gel(z, 1) = gadd(gsqr(x), y);
  gel(z, 2) = gadd(gsqr(y), x); z = gc_upto(ltop,z); /*@Ccom inefficient */
@eprog\noindent
leaves the results of \kbd{gsqr(x)} and \kbd{gsqr(y)} on the stack (and
lets \kbd{gc\_upto} update them for naught). In some sense, the best
version is
\bprog
  z = cgetg(3, t_VEC);
  ltop = avma; gel(z, 1) = gc_upto(ltop, gadd(gsqr(x), y));
  ltop = avma; gel(z, 2) = gc_upto(ltop, gadd(gsqr(y), x));
@eprog\noindent
...
similarly for $y$ and $z$. We compute $a = x_r*y_r$, $b=x_i*y_i$,
$c=(x_r+x_i)*(y_r+y_i)$, and then we have $z_r=a-b$,
$z_i=c-(a+b)$. The program is as follows:
%
\bprog
ltop = avma;
a = gmul(gel(x,1), gel(y,1));
b = gmul(gel(x,2), gel(y,2));
c = gmul(gadd(gel(x,1), gel(x,2)), gadd(gel(y,1), gel(y,2)));
d = gadd(a,b); z = cgetg(3, t_COMPLEX);
gel(z, 1) = gsub(a,b);
gel(z, 2) = gsub(c,d); z = gc_upto(ltop, z);
@eprog

\subsec{Random garbage collection}

The most important usage of \kbd{gc\_all} is to make sure loops don't
fill up the stack:
\bprog
  pari_sp av = avma;
  GEN x, y;
  while (...)
  {
    garbage(); x = anything();
    garbage(); y = anything();
    garbage(); gc_all(av, 2, &x, &y);
  }
@eprog
\noindent Here we assume that only \kbd{x} and \kbd{y} are needed from one
iteration to the next. As it can be costly to call \kbd{gc\_all} once for
each iteration, we may do this only when it is necessary:
\bprog
    /*@Ccom memory is running low (half spent since entry) */
    if (gc_needed(av,1)) gc_all(av, 2, &x, &y);
@eprog
...
point \kbd{av} is exhausted ($1/2$ for $n=1$, $2/3$ for $n=2$, etc.). The test
\tet{gc_needed}\kbd{(av,$n$)} becomes true whenever \kbd{avma} drops below
that address.

\subsec{Final comments}

PARI's manual garbage collection may seem cumbersome at first. But it is
flexible and very fast, and compares favorably with methods used in other
systems. Our benchmarks indicate that the price paid during garbage
collections, when properly used, is usually less than 1\%
of the total running time, which is quite acceptable!

It is of course harder on the programmer, and error-prone if you do not stick
to a consistent PARI programming style. At first, just use \kbd{gc\_GEN} or
\kbd{gc\_all} to fix up the stack for you. You can always optimize later when
you have sorted out exactly which routines are crucial and what objects need
to be preserved and their usual sizes. In any case, all garbage collections
routines run in almost linear complexity in the size of the recovered data.
...
statement (for instance \kbd{affii} or \kbd{affrr}).
...
a constant object. Most of the time you will use \kbd{GEN} objects as they
...
\fun{GEN}{cgetr}{long prec}

\noindent should be used instead of \kbd{cgetg(length, t\_INT)} and
\kbd{cgetg(prec2lg(prec), t\_REAL)} respectively. Finally
...
returns the nonnegative \typ{INT} whose expansion in base $2^{32}$
...
latter three functions are shallow: they do \emph{not} copy their arguments.
For instance
\bprog
  /*@Ccom stack-clean: components are universal objects */
  z = mkvecn(3, gen_1, gen_0, gen_2);

  /*@Ccom not OK for gc\_upto: stoi(3) creates component before root */
...
It is also very useful to \idx{copy} a PARI object, not just by moving around
a pointer as in the \kbd{y = x} example, but by creating a copy of the whole
tree structure. The function which does this is called \teb{gcopy}. Its
syntax is:
...
\kbd{y = x}, \kbd{y = gcopy(x)} and \kbd{y = shallowcopy(x)}.
...
garbage collection functions. The function which does this is called
...
\typ{REAL} actually increases by increments of \B bits.
...
sets \kbd{*e} to the exponent $\kbd{realprec(z)}-1-\kbd{expo}(z)$, so that $z = m / 2^e$.

Thus, the real number $3.5$ to accuracy \kbd{realprec(z)} is
...
\fun{void}{setprecp}{GEN z, long d} sets the $p$-adic precision of \kbd{z}
to \kbd{d}.
...
\item \tet{padic_p}$(z)$: the prime $p$.

\item \tet{padic_pd}$(z)$: $p^d$, where $d$ is the $p$-adic precision

\item \tet{padic_u}$(z)$: the $p$-unit $u$ modulo $p^d$
...
resp. constant, coefficient with respect to the main variable. Note that this
is a \emph{shallow} construction: no copy is made on the PARI stack so the
returned value is not safe for a basic \kbd{gc\_upto} call. Applied to any
other type than \typ{POL}, the result is unspecified. Those three functions
are still defined when the sign is $0$, see \secref{se:accessors} and
\secref{se:polynomials}.
...
\teb{setsigne}, \teb{varn}, \teb{setvarn} as for polynomials.
Beware: do \emph{not} use \teb{expo} and \teb{setexpo} on power series.

\fun{long}{valser}{GEN z} returns the valuation of \kbd{z}
This is defined even if \kbd{z} is equal to~0.

\fun{void}{setvalser}{GEN z, long e} sets the valuation of \kbd{z}
to \kbd{e}.
...
of type \typ{POL}, with variable of priority at least as high as that of the
...
destroyed by garbage collection). When you do not need a variable number
anymore, you can delete it using
...
\fun{GEN}{gp_read_str_bitprec}{const char *s, long bitprec} identical to
\kbd{gp\_read\_str\_prec}. DEPRECATED.
...
Note that \kbd{DEBUGLEVEL} is redefined in each code module, attaching it to
a particular debug domain (see \kbd{setdebug}).

\fun{void}{setalldebug}{long L} sets all \kbd{DEBUGLEVEL} incarnations (all
debug domains) to $L$.
...
\item \tet{forperm} to loop over permutations of $k$. See the documentation
  ...
  as a \typ{VECSMALL} or \kbd{NULL} when all permutations have been
...
@eprog\noindent Of course, the final \kbd{set\_avma(av)} could be replaced
by a \kbd{gc\_upto} call. Beware that swapping the \kbd{av2 = avma} and
\tet{forprime_init} call would be incorrect: the first \kbd{set\_avma(av2)}
would delete the \tet{forprime_t} structure!

\subsec{Parallel iterators}

These iterators loop over the values of a \typ{CLOSURE} taken at some data,
...
\item \tet{parforstep}. To initialize the analog of
\kbd{parforstep(i = a, b, s, ...)}, call

\fun{void}{parforstep_init}{parforstep_t *T, GEN a, GEN b, GEN s, GEN code}

initialize an iterator over the evaluation of \kbd{code} between $a$ and $b$,
with steps $s$ (see \kbd{forstep} documentation for the possibilities for $s$).

\fun{GEN}{parforstep_next}{parforstep_t *T}

returns a \typ{VEC} \kbd{[x,code(x)]} where $x$ is one of the evaluation
points and \kbd{code(x)} is the evaluation, \kbd{NULL} when all data have
been exhausted. Once it happens, \kbd{parforstep\_next} must not be called
anymore with the same initialization.

\fun{void}{parforstep_stop}{parforstep_t *T} needs to be called when leaving
the iterator before \kbd{parforstep\_next} returned \kbd{NULL}.
...
\subsec{parallel data processing}

Below \kbd{f} and \kbd{g} are \typ{CLOSURE}s of arity $1$.

\fun{GEN}{gen_parapply}{GEN f, GEN V} applies \kbd{f} to each components of $V$ in parallel. Used to implement
\kbd{parapply}.

\fun{GEN}{gen_parapply_percent}{GEN f, GEN V, long percent} as \kbd{gen\_parapply}, if \kbd{percent} not zero,
print percentage of completion. \kbd{percent} should be $0$ when \kbd{DEBUGLEVEL} is $0$.

\fun{GEN}{gen_parapply_slice}{GEN g, GEN V, long m}
splits $V$ in at most \kbd{m} chunks in arithmetic progressions,
and calls \kbd{g} on each in parallel, and reassembles the result.
\kbd{g} needs to apply \kbd{f} to each components of the chunk.
The result is identical to \kbd{gen\_parapply(f, V)} with a lower overhead.
The usual choice of \kbd{m} is \kbd{mt\_nbthreads()}.

\fun{GEN}{gen_parapply_slice_zv}{GEN g, GEN V, long m)}
As \kbd{gen\_parapply\_slice}, but \kbd{g} must return \typ{VECSMALL} and the
output is a \typ{VECSMALL} of \kbd{long} (\kbd{zv}).
