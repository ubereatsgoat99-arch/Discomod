% see https://inria.hal.science/hal-00934443 to simplify/improve the ulp calculus
...
\DeclareMathOperator{\LiTwo}{Li_2}
...
See \cite{lefevre:inria-00000026}.
...
truncate, then divide by $k$ and truncate; this gives the same answer as
...
The $k$th term is of the form $k! x^{-k-1}$. The smallest value is obtained for
...
$J_n(z)$; assuming those zeros are irrational, $Y_n(z)$ is thus NaN for
$z$ negative.
...
$x \in [2, +\infty[ $&$ \Re(\LiTwo(x)) $&$=$&$ \frac{\pi^2}{3}
- \frac{1}{2}\log^2(x) - \LiTwo\left(\frac{1}{x}\right) $\\
$x \in ]1, 2[ $&$ \Re(\LiTwo(x)) $&$=$&$ \frac{\pi^2}{6}-\log(x)\left[\log(x-1)
- \frac{1}{2} \log(x)\right] + \LiTwo\left(1 - \frac{1}{x}\right)  $\\
 &$ \LiTwo(1) $&$=$&$ \frac{\pi^2}{6} $\\
$x \in \left]\frac{1}{2}, 1\right[ $&$ \LiTwo(x) $&$=$&$ \frac{\pi^2}{6}
- \log(x) \log(1 - x) - \LiTwo(1 - x) $\\
 &$ \LiTwo(0) $&$=$&$ 0 $\\
$x \in [-1, 0[ $&$
\LiTwo(x) $&$=$&$ -\frac{1}{2} \log^2(1-x) - \LiTwo\left(\frac{x}{x - 1}\right)$\\
$x \in ]-\infty, -1[ $&$ \LiTwo(x) $&$=$&$ -\frac{\pi^2}{6} 
- \frac{1}{2} \log(1 -x) [2 \log(-x) - \log(1 - x)]
+ \LiTwo\left(\frac{1}{1 - x}\right).$
\end{tabular}


Assume first $0 < x \leq \frac{1}{2}$, the odd Bernoulli numbers being zero
(except $B_1 = -\frac{1}{2}$), we can rewrite $\LiTwo(x)$ in the form:
\[
\LiTwo(x) = - \frac{\log^2(1-x)}{4} + S(-\log(1-x))
\]
where  
\[
...
$\LiTwo\left(\frac{1}{x}\right) = \frac{1}{x} + O\left(\frac{1}{x^2}\right)$
(see below):
\[
\left|\LiTwo(x) + \frac{\log^2x}{2} - \frac{\pi^2}{3}\right| \leq \frac{2}{x}
\]
...
$\circ_n$ \\
Output: $\circ_n(\Re(\LiTwo(x)))$ if it can round exactly, a failure indicator
if not\\
$u \leftarrow \N(\log x)$\\
$v \leftarrow \N(u^2)$\\
$g \leftarrow \N(v/2)$\\
...
\[
\LiTwo(x) = -S\left(-\log(1-\frac{1}{x})\right)
...
\[
\LiTwo(x) = S(\log x) + \frac{\log^2 x}{4} - \log x \log(x - 1) + \frac{\pi^2}{6}
\]
...
When $x=1$, we have a simpler value $\LiTwo(1) = \frac{\pi^2}{6}$ whose
...
\[
\LiTwo(x) = - S(-\log x) - \log x \log(1 - x) + \frac{\log^2 x}{4}
+ \frac{\pi^2}{6}
\]
which is implemented as follows
\begin{quote}
...
\[
\LiTwo(x) = \sum_{n=0}^{\infty}\frac{x^k}{k^2}
\]
which is true for $|x| \leq 1$ [FIXME: ref].
If $x \leq 0$, we have $0 \leq \LiTwo(x) - x \leq \frac{x^2}{4}$ 
and if $x$ is positive, 
$0 \leq \LiTwo(x) - x \leq (\frac{\pi^2}{6}-1)x^2 \leq x^2 
\leq 2^{2\Exp(x)+1}\ulp(x)$.

 
When $x \in [-1, 0[$, we use the relation
\[
\LiTwo(x) = - S(-\log(1-x)) - \frac{\log^2(1-x)}{4}
\]
which is implemented as follows
\begin{quote}
...
$-\infty$:
\[
\left|\LiTwo(x) + \frac{\log^2(-x)}{2} + \frac{\pi^2}{3}\right| \leq 
\frac{1}{|x|}
\]
which gives the following algorithm:
\begin{quote}
  ...
  $\circ_n$ \\
  Output: $\circ_n(\LiTwo(x))$ if it can round exactly, a failure indicator
  if not\\
  $l \leftarrow \N(\log(-x))$ \\
  $f \leftarrow \N(l^2)$\\
  $g \leftarrow \N(f/2)$\\
...
we use the relation
\[
\LiTwo(x) = S\left(\log\left(1-\frac{1}{x}\right)\right) - \frac{\log^2(-x)}{4}
...
\url{https://web.archive.org/web/20090624123133/http://www-2.cs.cmu.edu/~adamchik/articles/catalan/catalan.htm}}:
