...
\li{in place bit operations}
   {bitclear{\rm, }bitflip{\rm, }bitset}
...
\li{is key $k$ in the dict? (set $v$ to $M(k)$)}
   {mapisdefined$(M,k,\{\&v\})$}
\li{evaluate $f$ at $M(k)$}{mapapply$(M, k, f)$}
...
\li{\dots for $i=a, a+s, \dots,b$}
   {parforstep$(i = a, \{b\}, s, f,\{r\}, \{f_2\})$}
...
Image, kernel, inverse and determinant modulo $D$:
\kbd{matimagemod}, \kbd{matkermod}, \kbd{matinvmod}
, \kbd{matdetmod}.
...
\li{Cholesky decomposition of $x$}{qfcholesky$(x)$}
...
\li{write $A = QH + R$, $H$ in HNF}{mathmodhnf$(A,H,\{\&Q\})$}
...
\li{find up to $m$ sols of \kbd{qfeval}$(x,y)\le b$}{qfminim$(x,b,m)$}
\li{\dots up to $m$ closest vectors to $t$}{qfcvp$(x,t,m)$}
\li{$v$, $v[i]:=$number of $y$ s.t. \kbd{qfeval}$(x,y)= i$}
    {qfrep$(x,B,\{\fl\})$}
...
\li{monic polynomial from roots $r$}{polfromroots$(r)$}
...
\li{draw ellipse}{plotarc$(w,lx,ly,\{\fl\})$}
