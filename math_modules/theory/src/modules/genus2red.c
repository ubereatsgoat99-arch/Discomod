...
struct igusa_p {
  long eps, eps2, tt, r1, r2, R, tame, abe, tor;
...
get_red(struct red *S, struct igusa_p *Ip, GEN polh, GEN p, long alpha, long r)
{
  ...
  switch(r)
  {
    case 0:
      ...
      S->tnum = 1;
      S->pages = "159-177";
      if (indice==0) Ip->abe += 1; else Ip->tor += 1;
      S->g = cyclic(indice);
      return indice ? indice: 1;
    case 6:
      if (alpha == 0) polh = ZX_unscale_divpow(polh, p, 3); /* H(px) /p^3 */
      indice = FpX_is_squarefree(FpX_red(polh,p), p)
               ? 0
               : val[6] - val[7] + val[8]/Ip->eps;
      S->t = stack_sprintf("I*{%ld}", indice);
         ...
         long Dmin, long R, struct igusa *I, struct igusa_p *Ip)
{
  ...
  { /* (r1,r2) = (0,0), (0,6), (6,0) or (6,6) */
    ...
    if (Ip->tt == 6)
    {
      ...
      if (Ip->r1 && alpha1 == 0) polh1 = ZX_unscale_divpow(polh1, p, 3);
    ...
    { /* Ip->tt == 7 */
      ...
      if (t60_1 == 60) polh1 = ZX_unscale_divpow(polh1, p, 3);
    ...
    if (Ip->tt == 6)
    {
      if (indice==0) { page = 170; Ip->abe = 1; Ip->tor = 0; }
      else           { page = 171; Ip->abe = 0; Ip->tor = 1; }
    } else           { page = 180; Ip->abe = 0; Ip->tor = 1; }
    Ip->type = stack_sprintf("[I{%ld}-I*{%ld}-%ld] page %ld",
                             indice,d-indice,R, page);
    return condp;
  }
...
genus2localred(struct igusa *I, struct igusa_p *Ip, GEN p, GEN polmini)
{
  ...
  if (Dmin == 3)
  {
    switch(Ip->tt)
    {
      ...
      case 3:
        Ip->type = "[I{2-1-0}] page 179";
        Ip->abe = 0; Ip->tor = 2;
        Ip->neron = cyclic(2);
        return 2;
      case 4:
        Ip->type = "[I{1-1-1}] page 182";
        Ip->abe = 0; Ip->tor = 2;
        Ip->neron = cyclic(3);
        return 2;
      case 5:
        ...
        Ip->type = "[I{0}-III-0] page 161"; Ip->neron = cyclic(2);
        Ip->abe = 1; Ip->tor = 0; return 2;
      case 6:
        if (equaliu(p,3)) pari_err_BUG("genus2localred [conductor]");
        Ip->type = "[I{1}-II-0] page 172"; Ip->abe = 0; Ip->tor = 1; Ip->neron = cyclic(1); return 3;
    }
...
genus2red(GEN PQ, GEN p)
{
  ...
  return gc_GEN(av, mkvec4(cond, facto, PQ, V));
}
